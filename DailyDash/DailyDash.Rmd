...
if (is.yaml.file('./DailyDash_cfg.yaml')) {
  ...
  config$server <- c('127.0.0.1\\BPSINSTANCE')
...
observeEvent(c(
  ...
  input$location),
  {
    if (!is.null(input$location)) { # only if initialized
      ...
      print(paste('redefined clinician list:', clinician_choice_list()))
      # note that 'ifelse' only returns result in the same 'shape' as the comparison statement
      #choice_list <- isolate(clinician_choice_list)
      #chosen_list <- isolate(input$clinicians) # retain previous selections
      #updateCheckboxGroupInput(session, 'clinicians', choices = choice_list, selected = chosen_list)
    }
  })
...
update_database_details <- observeEvent(input$update_database, {
  tmp_pool <- tryCatch(dbPool(odbc::odbc(), driver = "SQL Server",
                       ...
                       error = function(e) {sendSweetAlert(title = 'Unable to connect to database',
                                                           type = 'error',
                                                           html = TRUE,
                                                           text = paste ('<font color="red">', e, '</font><br>',
  ...
  if (is.environment(tmp_pool)) {
      ...
      sendSweetAlert(title = 'Connected!', type = 'success')
  }
