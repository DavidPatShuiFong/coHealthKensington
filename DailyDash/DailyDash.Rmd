...
wellPanel(
  ...
  helpText("After adjusting datebase details, click the 'Update' button",
           "Changed settings are not used until DailyDash is restarted.")
)

update_database_details <- observe({
...
locations.insert.callback <- function(data, row) {
  mydata <- rbind(data, config$practice_locations)
  return(mydata)
}

locations.update.callback <- function(data, olddata, row) {
	config$practice_locations[row,] <- data[1,]
	return(mydata)
}
