...
green_tag_label <- function(content) {
  paste('<span class="huge green positive ui tag label">', toString(content), '</span>', sep = "")
}
...
appointments_billings_sameday <- reactive({
  appointments_billings() %>%
    ...
    mutate(MBSITEM = paste('<span class="huge green positive ui tag label">', MBSITEM, '</span>', sep = "")) %>%
    ...
    summarise(Billings = toString(MBSITEM)) %>%
    ungroup()
  })
