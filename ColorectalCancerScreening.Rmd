...
Approximately 75% of eligible patients each week seen at the practice leave without a record of immunization, it is proposed that patients booked in each day are identified for potential Zostavax immunization eligibility.
...
Main data source is extracted using [SQL code](SQLyfo) in Best Practice[^3].

[^3]: Similar data is available from PenCAT, which is also plotted for comparison purposes. PenCAT screening rates are lower, probably because PenCAT's search tools seems to only search for results in 'Investigation rerports', when many results have been filed in 'Correspondence'. 
